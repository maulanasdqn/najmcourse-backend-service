use super::{
	MonthlyScoreDto, SessionsDetailResponseDto, SessionsDetailSchema,
	SessionsResponseDto, SessionsSchema, StudentStatsResponseDto, TestTakenDto,
};
...
impl<'a> SessionsRepository<'a> {
	...
	pub async fn query_student_stats(
		&self,
		user_id: &str,
	) -> Result<StudentStatsResponseDto> {
		let db = &self.state.surrealdb_ws;
		...
		let mut test_scores: std::collections::HashMap<
			String,
			(String, String, String, i32, String, String),
		> = std::collections::HashMap::new();
		let mut monthly_scores: std::collections::HashMap<String, Vec<i32>> =
			std::collections::HashMap::new();
		...
		let tests_taken: Vec<TestTakenDto> = test_scores
			...
			.map(
				|(
					test_id,
					(test_name, category, session_id, score, session_name, taken_at),
				)| {
					TestTakenDto {
						test_id,
						test_name,
		...
		let average_score_overall = if total_tests_taken > 0 {
			tests_taken.iter().map(|t| t.score as f64).sum::<f64>()
				/ total_tests_taken as f64
		} else {
			0.0
		};
