use super::{
	AdminDashboardStatsResponseDto, UserStatsDto, ExaminationStatsDto, 
	PerformanceStatsDto, ContentStatsDto, SystemStatsDto, UsersByRoleDto,
	RegistrationTrendDto, TestsByCategoryDto, SessionParticipationDto,
	MonthlyPerformanceDto, TopStudentDto, QuestionDifficultyDto,
	SubjectDistributionDto, DatabaseMetricsDto, GrowthMetricsDto,
	MonthlyGrowthDto, DataIntegrityDto
};
use anyhow::Result;
...
impl<'a> AdminStatsRepository<'a> {
	...
	pub async fn query_admin_dashboard_stats(&self) -> Result<AdminDashboardStatsResponseDto> {
	...
	async fn query_user_stats(&self) -> Result<UserStatsDto> {
		...
		let user_count_data = user_counts.get(0).ok_or_else(|| anyhow::anyhow!("No user count data"))?;
		...
		let registration_trends: Vec<Value> = db.query(registration_trends_query).await?.take(0)?;
		...
		Ok(UserStatsDto {
			...
			completed_profiles: user_count_data["completed_profiles"].as_u64().unwrap_or(0),
			completed_payments: user_count_data["completed_payments"].as_u64().unwrap_or(0),
	...
	async fn query_examination_stats(&self) -> Result<ExaminationStatsDto> {
		...
		let exam_count_data = exam_counts.get(0).ok_or_else(|| anyhow::anyhow!("No exam count data"))?;
		...
		let tests_by_category: Vec<Value> = db.query(tests_by_category_query).await?.take(0)?;
		...
		let session_participation: Vec<Value> = db.query(session_participation_query).await?.take(0)?;
		let session_participation_data: Vec<SessionParticipationDto> = session_participation
			.iter()
		...
		Ok(ExaminationStatsDto {
			...
			total_answers_submitted: exam_count_data["total_answers_submitted"].as_u64().unwrap_or(0),
	...
	async fn query_performance_stats(&self) -> Result<PerformanceStatsDto> {
		...
		let monthly_active: Vec<Value> = db.query(monthly_active_query).await?.take(0)?;
		let monthly_active_count = monthly_active.get(0)
		...
		let performance_data = performance.get(0).ok_or_else(|| anyhow::anyhow!("No performance data"))?;
		...
		let monthly_performance: Vec<Value> = db.query(monthly_performance_query).await?.take(0)?;
		...
		Ok(PerformanceStatsDto {
			...
			total_test_attempts: performance_data["total_test_attempts"].as_u64().unwrap_or(0),
			overall_average_score: performance_data["overall_average_score"].as_f64().unwrap_or(0.0),
	...
	async fn query_content_stats(&self) -> Result<ContentStatsDto> {
		...
		let content_count_data = content_counts.get(0).ok_or_else(|| anyhow::anyhow!("No content count data"))?;
		...
		let question_difficulty: Vec<Value> = db.query(question_difficulty_query).await?.take(0)?;
		...
		for item in question_difficulty {
			...
			let entry = difficulty_counts.entry(difficulty.to_string()).or_insert((0, 0.0));
			entry.0 += 1;
		...
		let question_difficulty_data: Vec<QuestionDifficultyDto> = difficulty_counts
			...
			.map(|(difficulty, (count, total_rate))| QuestionDifficultyDto {
				difficulty,
				count,
				correct_rate: if count > 0 { total_rate / count as f64 } else { 0.0 },
			})
			.collect();
		...
		let subject_distribution: Vec<Value> = db.query(subject_distribution_query).await?.take(0)?;
		let subject_distribution_data: Vec<SubjectDistributionDto> = subject_distribution
			.iter()
		...
		Ok(ContentStatsDto {
			...
			average_correct_rate: content_count_data["average_correct_rate"].as_f64().unwrap_or(0.0),
	...
	async fn query_system_stats(&self) -> Result<SystemStatsDto> {
		...
		let db_metrics_data = db_metrics.get(0).ok_or_else(|| anyhow::anyhow!("No database metrics data"))?;
		...
		let growth_data = growth.get(0).ok_or_else(|| anyhow::anyhow!("No growth data"))?;
		
		let new_users_this_month = growth_data["new_users_this_month"].as_u64().unwrap_or(0);
		let new_users_last_month = growth_data["new_users_last_month"].as_u64().unwrap_or(0);
		let new_tests_this_month = growth_data["new_tests_this_month"].as_u64().unwrap_or(0);
		
		let user_growth_rate = if new_users_last_month > 0 {
			((new_users_this_month as f64 - new_users_last_month as f64) / new_users_last_month as f64) * 100.0
		} else {
			0.0
		};
		...
		let monthly_growth: Vec<Value> = db.query(monthly_growth_query).await?.take(0)?;
		...
		let integrity_data = integrity.get(0).ok_or_else(|| anyhow::anyhow!("No integrity data"))?;
		
		let total_records = database_metrics.total_users + database_metrics.total_tests + 
			database_metrics.total_sessions + database_metrics.total_questions;
